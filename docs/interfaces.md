...
> deliverable plugs into; the recipe/`channels.yaml` contract lives in the
...
| `POST /api/frames/create` | Mint a frame dir and spawn its persistent agent (via taskpilot's `spawner_cli.py`). |
