...
## Current dashboard state (as of 2026-06-04)

- `dashboard/server/server.py` — FastAPI. Endpoints: `/api/health`, `/api/panes`, surface mindframe listing (`/api/frames` — frame dirs holding an `index.html`), System overview feeds (`/api/events`, `/api/agents`, `/api/capabilities`), `POST /api/dashboard-event` (dispatcher proxy, bearer from `~/.mindframe/secrets/dispatcher-bearer.token`), vaults (`/api/vaults`, `/api/vaults/<name>/entries|graph`, `POST .../share`), sources (`/api/sources`, connect/disconnect) + **`/api/connections` (live discovery)**, shares, `/artifacts/<sid>/<path>`, SPA fallback. **Block-stream endpoints (`/api/frame/<id>[/blocks|/stream]`, `POST /api/frames`/`/api/prompt`, `/api/suggestions`) removed 2026-06-04.**
- `dashboard/public/` — Space Grotesk / Source Serif 4 / JetBrains Mono, warm dark, gold accent. Routes `/` (home: vaults + sources) and `/system` (bundle overview: events, agents, mindframes, skills+MCPs, knowledge bases). Per-mindframe viewing on the surface model is the next step.
